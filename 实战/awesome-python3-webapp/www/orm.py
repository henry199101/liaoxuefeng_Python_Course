...
class ModelMetaclass(type):
	def __new__(cls, name, bases, attrs): # 当前准备创建的类的对象；类的名字；类继承的父类集合；类的方法集合。
		...
		attrs['select'] = 'select `%s`, %s from %s'%s(primarykey, ',',.join(escaped_fields), tableName)
		...
		attrs['__updates__'] = 'update `%s` set %s where `%s`=?'%(tableName, ','.join(map(lambda f: '`%s`=?'%(mappings.get(f).name or f),fields)),primarykey) # 查询列的名字，也看一下在Field定义上有没有定义名字，默认None
		attrs['__deletes__'] = 'delete from `%s` where `%s`=?'%(tableName, primarykey)
		return type.__new__(cls, name, bases, attrs)
...
class Model(dict, metaclass = ModelMetaclass):
	...
	def __getattr__(self, key):
		try:
			...
			raise AttriuteError('"Model" object has no attribute: %s'%key)
